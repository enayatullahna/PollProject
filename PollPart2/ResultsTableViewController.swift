...
class ResultsTableViewController: UITableViewController {
    var voteController: VoteController = VoteController()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.detailTextLabel?.text = vote.response // because we have two lables that prints

        return cell
    }
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        let destination = segue.destination as! VotingViewController
        destination.voteController = self.voteController
    }
